...
const SYSTEM_PROMPT =
  ...
  'You are Vizi, an AI guitar tutor.';

const REMINDER_PROMPT =
  process.env.REMINDER_PROMPT || '';

const SONG_PROMPT =
  process.env.SONG_PROMPT || '';

const STRUMMING_PROMPT =
  process.env.STRUMMING_PROMPT || '';

const SOLOING_PROMPT =
  process.env.SOLOING_PROMPT || '';

const CURRICULUM_PROMPT =
  process.env.CURRICULUM_PROMPT || '';

// ─── Anthropic prompt caching ────────────────────────────────────────────────
// Cache the large, stable Vizi prompt prefix.
//
...
//
// remains in "messages" and therefore does NOT invalidate the cached
...
// Anthropic system prompts can be supplied as content blocks.
// The cache marker tells Anthropic that this stable prefix may be reused.
...
// Shows whether the request created or reused the cache.
//
// Typical first request:
// created:9000 read:0
//
// Typical following request:
// created:0 read:9000
...
// so Vizi always knows their level.
//
// History is left clean so old codes don't accumulate.
function injectProgress(messages, progress, steps) {
  if (
    !progress ||
    !/^[0-9]{6}$/.test(String(progress))
  ) {
    return messages;
  }
  ...
  if (
    ...
    header +=
      `\nCURRENT STEPS ` +
      `(already resolved from the code — teach THESE exact steps; ` +
...
//
// The fretboard can't be reached directly from the HTTPS app.
//
// Commands are queued here and the ESP32 polls Railway over its own
...
function enqueueFretboardCommands(commandsStr) {
  ...
  commandsStr.split('|').forEach(c => {
    ...
    // Client/state commands are never sent to the ESP32.
    if (/^(OPEN|PROGRESS)\b/i.test(cmd)) {
      return;
    }

    // Auto-Hold static scale displays.
...
// ESP32 polls this endpoint several times per second.
...
// Manual command enqueue for testing.
...
app.post('/fretboard-clear', (req, res) => {
  ...
  console.log(
    `[fretboard-clear] Flushed ${cleared} queued command(s)`
  );
...
// GET version for firmware/manual use.
app.get('/fretboard-clear', (req, res) => {
  ...
  console.log(
    `[fretboard-clear] Flushed ${cleared} queued command(s) via GET`
  );
...
app.post('/reset', (req, res) => {
  ...
  console.log(
    'Conversation history reset via POST'
  );
...
app.get('/reset', (req, res) => {
  ...
  console.log(
    'Conversation history reset via GET'
  );
...
// ─── STT + Claude + TTS combined ─────────────────────────────────────────────
//
// POST /stt-claude-tts
//
// Request:
// {
//   "audio": "<base64 LINEAR16>",
//   "sampleRate": 17000,
//   "mode": "general"
// }
//
...
const STRUM_PATTERNS = {
  ...
  'Pattern 2 — Down Up': {
    ...
    arrows: '↓ ↑ ↓ ↑',
    counts: '1 and 2 and',
    spoken: 'down, up, down, up'
  },

  'Pattern 3 — Common Pop Rock': {
    name: 'Common Pop Rock',
    arrows: '↓ ↓ ↑ ↑ ↓ ↑',
    counts: '1 2 and and 4 and',
    spoken: 'down, down, up, up, down, up'
  },
...
app.get(
  ...
  (req, res) => {
    ...
    message +=
      'You now have this song loaded. ' +
      'Follow your Song Mode initial response rules exactly. ' +
      'Your spoken introduction must come first, then append the CAPO command ' +
      'as a pipe command at the very end of your response.';
    ...
    const claudeBody =
      JSON.stringify({
        ...
        system:
          ...
          `Your job is to extract chord and tab information from uploaded music and ` +
          `return it as structured JSON. Always respond with ONLY valid JSON — ` +
...
// Safely coerce Claude's BPM into a reasonable integer.
